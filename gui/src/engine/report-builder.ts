...
function reportShell(kicker: string, docTitle: string, heading: string, meta: string, bodyHtml: string): string {
...
export function buildPatientReportHTML(
  ...
  return reportShell("PRISM · CLINICAL DECISION SUPPORT", `#${id}_Risk Report_PRISM`, "Risk Report", `Generated ${escapeHtml(timestamp)} · Report ID: ${id}`, body);
}
...
export function buildDashboardReportHTML(): string {
  ...
  const rowify = <T,>(arr: T[], cols: ((d: T) => string)[]) =>
    arr.map((d) => `<tr>${cols.map((c) => `<td>${escapeHtml(c(d))}</td>`).join("")}</tr>`).join("");

  const holdoutRows = rowify(holdoutAuroc, [(d) => d.name.replace("\n", " "), (d) => d.value.toFixed(3)]);
  const cvRows = rowify(cvAuroc, [(d) => d.name.replace("\n", " "), (d) => `${d.value.toFixed(3)} \u00b1 ${d.sd.toFixed(3)}`]);
  ...
  return reportShell("PRISM · CLINICAL DECISION SUPPORT", `#${id}_Performance Summary_PRISM`, "Performance Summary", `Generated ${escapeHtml(timestamp)} · Report ID: ${id}`, body);
}
