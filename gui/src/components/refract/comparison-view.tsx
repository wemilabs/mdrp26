...
export function ComparisonView({ entries }: { entries: [SavedAssessment, SavedAssessment] }) {
  const [a, b] = [...entries].sort((x, y) => x.savedAt - y.savedAt);
  const deltaPts = (b.probability - a.probability) * 100;
  const factorNames = [...new Set([...a.ranked.slice(0, 6), ...b.ranked.slice(0, 6)].map((r) => r.name))];
    ...
    <div className="mt-4 rounded-xl border border-prism-border bg-prism-bg p-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {[a, b].map((entry) => {
          ...
          return (
            <div key={entry.id} className="rounded-lg border border-prism-border bg-white p-3.5">
              <div className="truncate text-xs font-semibold text-prism-text">{entry.label}</div>
              <div className="mb-2 text-[10.5px] text-prism-muted-2">{new Date(entry.savedAt).toLocaleString()}</div>
              <div className="mb-3 flex items-baseline gap-2">
                <span className="font-display text-2xl font-bold" style={{ color: tier.colorVar }}>
      ...
      <div className="mt-4">
        <div className="mb-1.5 text-xs font-bold text-prism-text">Per-factor change (later &minus; earlier)</div>
          ...
          {factorNames.map((name) => {
            ...
            return (
              <div key={name} className="flex items-center justify-between text-xs">
                ...
                <span className={`font-semibold ${delta > 0 ? "text-prism-red" : delta < 0 ? "text-prism-seafoam" : "text-prism-muted-2"}`}>
                  {delta >= 0 ? "+" : ""}
